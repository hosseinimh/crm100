...
import { organizationsPage as strings } from "../../../../constants/strings";
...
const addAction = () => {
    _navigate(`${basePath}/organizations/add`);
};

const editAction = (item) => {
    if (!isNaN(item?.id) && item?.id > 0) {
        _navigate(`${basePath}/organizations/edit/${item.id}`);
    }
};
...
const fetchOrganizations = async (data = null) => {
